...
def generateMap(map_param):
    ...
    # Sort and find the indices of the top x lowest non-zero tree density squares
    top_x_lowest_density_indices = np.argsort(non_zero_tree_density)[:map_param]
    low_density_lat_idx, low_density_lon_idx = non_zero_density_indices[0][top_x_lowest_density_indices], \
        non_zero_density_indices[1][top_x_lowest_density_indices]
    ...
    map_path = os.path.join(os.getcwd(), 'belfast_greenery_map.html')
    section_grid_map.save(map_path)
